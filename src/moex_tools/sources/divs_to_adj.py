...
def prepare_files():
    ...
    bcs_pl = (
        ...
        .rename(
            {
                "Дата закрытия реестра": "record_date",
                "Последний день для покупки акций": "last_buy_date",
                "Размер дивиденда": "size",
                "Цена акции на закрытие": "close",
                "ISIN": "isin",
            }
        )
        .with_columns(
            pl.col("record_date").cast(pl.Date),
            pl.col("last_buy_date").cast(pl.Date),
            pl.col("size").round(4),
            pl.col("close").round(4),
        )
    )
    bcs_pl = bcs_pl.group_by(["last_buy_date", "ticker", "close"]).agg(pl.col("size").sum())
    ...
    fm_pl = pl.read_parquet(path / "financemarker_dividends.parquet").with_columns(
        pl.col("record_date").cast(pl.Date),
        pl.col("exdiv_date").cast(pl.Date),
        pl.col("registry_date").cast(pl.Date),
        pl.col("size").round(4),
    )
...
def prep_bcs(dfs):
    ...
    dfs["bcs"] = bcs_pl["last_buy_date", "ticker", "bcs_div"]
...
def divs_vola(df, first_col: str, second_col: str, threshold: float):
    cur_df = (
        df.with_columns(divs_mean=pl.mean_horizontal(first_col, second_col))
        .with_columns(
            (
                (
        ...
        .with_columns((pl.col("divs_vol") / pl.col("divs_mean")).alias("divs_vol_ratio"))
    )
    ...
    cur_df = cur_df.with_columns(
        pl.when(
            (pl.col(first_col) != 0)
            & (pl.col(second_col) != 0)
            & (pl.col("divs_vol_ratio") < threshold)
...
def create_join_divs(df, first_col: str, second_col: str, div_df):
    join_df = (
        ...
        .filter(pl.col("DATE_other").is_not_null())
        .with_columns(time_diff=(pl.col("DATE") - pl.col("DATE_other")).dt.total_days())
        .filter((pl.col("time_diff") > 0) & (pl.col("time_diff") < 30))
    )
    check = join_df.filter(join_df["SECID", "DATE"].is_duplicated())
    assert (
        len(check) == 0
    ), f"The base has duplicates after filtration. Need to reduce time_diff range:\n {check}"
...
def connect_divs_to_base(dfs):
    ...
    div_df = (
        df.join(bcs_pl, left_on=["SECID", "DATE"], right_on=["ticker", "last_buy_date"], how="left")
    ...
    null_divs = null_divs[
        "SECID", "DATE", "adj_factor", "true_div", "bcs_div", "tink_div", "fm_div"
    ]
...
def make_adj_prices(div_df):
    div_df = (
        ...
        .with_columns(div_adj=((pl.col("CLOSE_adj") - pl.col("true_div")) / pl.col("CLOSE_adj")))
        .with_columns(div_adj_cumprod=pl.col("div_adj").cum_prod().over("SECID"))
        .with_columns(
            [
        ...
        .sort(["SECID", "DATE"])
        .drop(["adj_factor", "div_adj", "div_adj_cumprod"])
    )
...
def run_divs_to_adj():
    print("Creating split-adjusted OHLC dataset by divs...")
    dfs = prepare_files()
    dfs = prep_bcs(dfs)
    dfs = prep_tink(dfs)
    dfs = prep_fm(dfs)
    ...
    div_df.write_parquet(settings.data_dir / "moex_splits_divs.parquet", compression="lz4")
